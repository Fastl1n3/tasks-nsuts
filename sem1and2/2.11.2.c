...
int cost[N][3] = { 0 };
int D[N] = { 0 };
int deikstra(Ver** A, int v0, int s2, int n, FILE* f, int* dad, int* S, int action) {
	if (A[v0] == NULL)
		return 0;
	for (int i = 1; i <= n; i++) {
		D[i] = INT_MAX;
...
int main() {
	...
	int requests0, requests1;
	for (int i = 0; i < p; i++) {
		fscanf(f1, "%d%d", &requests0, &requests1);
		int* dad = (int*)calloc(n + 1, sizeof(int));
		int* S = (int*)calloc(n + 1, sizeof(int));
		int ans = deikstra(A, requests0, requests1, n, f2, dad, S, 1);
		free(dad);
		free(S);
	}
	Floyd(n);
